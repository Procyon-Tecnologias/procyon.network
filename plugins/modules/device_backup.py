...
short_description: Backup huawei device configuration from network devices over
network_cli
options:
  defaults:
    description:
    - The I(defaults) argument will influence how the running-config is
      collected from the device.  When the value is set to true, the command
      used to collect the running-config is append with the all keyword.
      When the value is set to false, the command is issued without the all
      keyword.
    default: no
    type: bool
  filename:
  ...
  dir_path:
    ...
    - This option provides the path ending with directory name in which the
      backup configuration file will be stored. If the directory does not exist
      it will be first created and the filename is either the value of
      (filename) or default filename as described in (filename) options
      description.
      If the path value is not given in that case a I(backup) directory will be
      created in the current working directory and backup configuration will be
      copied in C(filename) within I(backup) directory.
    type: path
"""
    ...
    for feature in feature_list:
        if module.params[feature]:
            ...
            if supports_feature is None:
                module.fail_json(
                    msg=(
                      "This platform does not specify whether %s is supported "
                      "or not. "
                      "Please report an issue against this platform's cliconf "
                      "plugin."
                    ) % feature
                )
